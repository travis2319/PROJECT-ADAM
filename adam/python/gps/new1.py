...
def send_to_udp(data):
    try:
        ...
        binary_data = struct.pack('!ddd',
                                  float(data['timestamp']),
                                  float(data['latitude']),
                                  float(data['longitude']))
        print(f"Attempting to send binary data")
        udp_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        udp_socket.sendto(binary_data, (udp_host, udp_port))
        print("Binary data sent to UDP socket")
...
while running:
    try:
        ...
        if line.startswith('$GPGGA'):
            ...
            data = {
                'timestamp': time.time(),  # Changed to float
