...
The literature survey directly informed the ADAM System's design by leveraging standardized protocols, embedded systems, and data encryption to ensure accurate, reliable, and secure diagnostics for real-time vehicle monitoring and forensic support.
...
\newline
(IEEE Xplore) &
This paper presents DrivingStyles, a platform designed to analyze and improve driving habits for reducing fuel consumption and greenhouse gas emissions. The system uses real-time data from vehicle Electronic Control Units (ECUs) through OBD-II Bluetooth interfaces and smartphone technology to evaluate driving behavior and fuel efficiency.
\newline
...
\newline
(IEEE Xplore) &
This paper explores Fuzzy Logic (FL) and Random Forest (RF) algorithms to estimate vehicle fuel consumption for developing an IoT-enabled driving game that provides real-time feedback to promote efficient driving habits using the enviroCar database.
\newline
The research uses real-time vehicle data collected through OBD-II interfaces, focusing on throttle position sensor (TPS), RPM, vehicle speed, and fuel consumption across various car models and driving conditions. FL uses "if-then" rules for understandable feedback, while RF applies machine learning for high-accuracy predictions.
\newline
Results show RF outperforms FL with higher R² (0.896 vs. 0.65) and lower MSE (1.506 vs. 4.745), identifying speed and RPM as critical predictors. Combining FL's intuitive coaching with RF's precise estimates enhances IoT-enabled driving games and scales to other real-time IoT applications.
&
...
\newpage
\vspace*{1cm} % Adds vertical space at the top of the new page

\section{CHAPTER SUMMARY}
\begin{enumerate}
    \item The introduction presents a smart automotive diagnostic and activity monitoring solution that enhances safety, maintenance efficiency, and forensic investigation using Raspberry Pi, machine learning, and mobile integration.
    \item The project scope includes real-time engine diagnostics via the OBD-II interface, driving behavior analysis for insurance and traffic safety, and secure black-box-like accident data storage for forensic support.
    \item The literature survey highlights the importance of standardized OBD-II protocols, embedded system integration with microcontrollers, and data encryption to ensure accurate diagnostics and secure data handling.
    \item By leveraging standardized diagnostic protocols it ensures compatibility with various vehicle systems, securing wireless communication channels, and maintaining reliable data collection in real-time scenarios.
    \item The purpose of the ADAM System is to empower users with informed vehicle insights, promote safer driving, assist in post-accident investigations, and establish transparency in automotive diagnostics and maintenance.
\end{enumerate}