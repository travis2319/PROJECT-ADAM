...
\section{CHAPTER SUMMARY}
    \begin{enumerate}
        ...
        \item \textbf{Raspberry Pi 5 – Edge Processing Unit:}  The \textbf{Raspberry Pi 5} acts as the central processor, interfacing with GPS, accelerometer, gyroscope, and vibration sensors to gather motion and location data. It also collects engine metrics like RPM, speed, and temperature via the OBD-II interface using a Python library.
        ...
        \item \textbf{FastAPI Server – API and Analytics Layer:}  The \textbf{FastAPI server} functions as the application’s API layer, receiving structured data from the Raspberry Pi, validating it, and performing analytics like predictive maintenance using machine learning models. The processed data is then sent to the mobile application for display.
        \item \textbf{Mobile Application – User Interface Layer:}  The mobile application acts as the primary user interface, connecting to the \textbf{FastAPI backend} to display real-time vehicle data, historical trends, and alerts for issues like overheating or abnormal driving behavior. It supports \textbf{role-based access}, offering tailored views for vehicle owners, fleet managers, and insurance personnel.
    \end{enumerate}