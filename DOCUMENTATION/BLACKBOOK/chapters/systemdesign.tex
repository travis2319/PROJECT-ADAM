...
\begin{enumerate}
    \item {Sensor and Hardware Layer:} 
        At the core of the system lies the Raspberry Pi 5, which functions as the primary edge computing device. It interfaces with multiple onboard sensors and modules, including:
            ...
            \end{table}
            \\
    \item {Edge Device – Raspberry Pi 5} 
    ...
    \item {API Interface – Golang server}
    ...
    \item {Database Layer – PostgreSQL}
    ...
    \item {Backend Logic – FastAPI Server}
    ...
    \item {User Interface – Mobile Application (Expo Framework)}
        Developed using Expo (React Native), the mobile application provides a user-friendly interface for various stakeholders, including vehicle owners, fleet managers, and insurance agents. It enables users to:
        \begin{itemize}
            \item {View real-time and historical vehicle data}
            \item {Receive alerts and notifications}
            \item {Access performance analytics and incident logs.}
            \item {Export data for insurance or forensic purposes.}
        \end{itemize}
